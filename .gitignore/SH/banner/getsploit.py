...
# Несоколько важных моментов:
#                       1. Путь к API-key: "C:\Users\Admin\.getsploit\vulners.key"
...
#Импорт необходимых моделей ORM
from .models import Table_Port, DownloadProof
...
class sploitVulners(vulners.Vulners):
    ...
    def searchExploit(self, query, limit, lookup_fields=None, offset=0, fields=None):
...
'''
    def downloadGetsploitDb(self, full_path):
        ...
        return True
'''
# Функция по грамотному созданию названий пруфов
def slugify(value):
    """
...
'''
def start(searchQuery, limit, id):

    #Проверка на наличие файла vulners.key, если его нет, то предлагается ввести Ваш API-key
    ...
    try:
        #Создание объекта класса sploitVulners с передачей параметра API-key
    ...
    # Проверка на наличие локальной БД, нахер это
    '''
    if LOCAL_SEARCH_AVAILABLE and options.local:
        ...
        '''
    finalQuery, searchResults = vulners_lib.searchExploit(searchQuery, limit)


    #Отрисовка талицы в консоле
    '''
...
'''
    for bulletin in searchResults:
        ...
        '''
         ...
         # ОТСЮДА начинается код по сохранению найденных файлов
        #if options.mirror:
        dirname = os.path.dirname(__file__)
        pathName = dirname +  '/%s' % slugify(searchQuery)


            # Put results it the dir
        if not os.path.exists(pathName): # Проверка на наличие директории для сейва
            os.mkdir(pathName)
        import codecs, datetime
        with codecs.open("%s/%s.txt" % (pathName, slugify(bulletin.get('id'))), 'w', "utf-8") as exploitFile:
            ...
            #Записываем в БД
            table_port = Table_Port.objects.get(id=id)
            Proof = DownloadProof(port = table_port,
                                    document = exploitData,
                                    description = slugify(bulletin.get('id')) + '.py')
            Proof.save()
    ...
    # Для красивого вывода в консоле
    '''
    else:
        # Text output
